...
def main():
    st.title("SDG Proposal Evaluator")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Evaluate Proposal"):
            with st.spinner("Evaluating with ChatGenAI..."):
                ...
                try:
                    progress_text.text("Preparing evaluation...")

                    progress_bar.progress(25)
                    progress_text.text("Sending data to ChatGenAI...")
                ...
                if evaluation_result:
                    ...
                    total_score = sum(int(data['score']) for data in parsed_results.values())
                    average_score = total_score / len(parsed_results)
                    overall_percentage = (average_score / 10) * 100  # Convert to percentage

                    st.metric(label="Overall SDG Score", value=f"{overall_percentage:.2f}%")  # Display with Streamlit's metric
